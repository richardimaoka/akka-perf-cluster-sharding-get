...
echo "Creating the Akka cluster seed-node EC2 instance"
AKKA_SEED_NODE_SETTINGS=$(echo "$EC2_SETTINGS" | jq -c '.akka_seed_node_instance')
AKKA_SEED_NODE_INSTANCE_TYPE=$(echo "$AKKA_SEED_NODE_SETTINGS" | jq -r '.instance_type')
AKKA_SEED_NODE_INSTANCE_IP_ADDRESS_V4=$(echo "$AKKA_SEED_NODE_SETTINGS" | jq -r '.ip_address_v4')
AKKA_SEED_NODE_INSTANCE_SUBNET=$(echo "$AKKA_SEED_NODE_SETTINGS" | jq -c '.subnet')
AKKA_SEED_NODE_INSTANCE_SUBNET_ID=$(echo "${DESCRIBED}" | jq -c ".Stacks[0].Outputs[] | select(.OutputKey == $AKKA_SEED_NODE_INSTANCE_SUBNET) | .OutputValue")
...
aws ec2 run-instances \
  ...
  --instance-type "${AKKA_SEED_NODE_INSTANCE_TYPE}" \
  --key-name "demo-key-pair" \
  --iam-instance-profile "Name=${IAM_INSTANCE_PROFILE_SSM}" \
  --user-data "file://user-data.sh" \
  --network-interfaces "AssociatePublicIpAddress=true,DeviceIndex=0,PrivateIpAddress=${AKKA_SEED_NODE_INSTANCE_IP_ADDRESS_V4},Groups=${SECURITY_GROUP},SubnetId=${AKKA_SEED_NODE_INSTANCE_SUBNET_ID}" \
  --tag-specifications "ResourceType=instance,Tags=[{Key=Name,Value=akka-seed-node},{Key=role,Value=seed-node},{Key=exec-id,Value=${EXEC_UUID}}]"
...
SEED_NODE_IPV4=$(echo "$EC2_SETTINGS" | jq -r ".akka_seed_node_instance.ip_address_v4")
